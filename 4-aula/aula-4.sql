...
SELECT m.nome, m.cpf
FROM (SELECT nome, cpf FROM medicos) m
JOIN (SELECT cpf FROM pacientes) p ON m.cpf = p.cpf;

-- 2) Buscar nomes de funcionários e de médicos (exibir pares de nomes) que residem na mesma cidade
SELECT f.nome, m.nome
FROM (SELECT nome, cidade FROM funcionarios) f
JOIN (SELECT nome, cidade FROM medicos) m ON f.cidade = m.cidade;

-- 3) Buscar o nome e idade dos médicos que têm consulta marcada com a paciente cujo nome é Ana
SELECT m.nome, m.idade
FROM (SELECT codm, nome, idade FROM medicos) m
JOIN (SELECT codm, codp FROM consultas) c ON m.codm = c.codm
JOIN (SELECT codp, nome FROM pacientes) p ON c.codp = p.codp
WHERE p.nome = 'Ana';
...
FROM (SELECT codp, nome FROM pacientes) p
NATURAL JOIN (SELECT codp FROM consultas WHERE hora > '14:00:00') c;
...
FROM (SELECT nroa, andar FROM ambulatorios) a
NATURAL JOIN (SELECT codm, nroa FROM medicos) m
NATURAL JOIN (SELECT codm FROM consultas WHERE data = '12/10/2020') c;
...
FROM (SELECT nroa FROM ambulatorios) a
LEFT JOIN (SELECT nroa, codm, nome FROM medicos) m ON a.nroa = m.nroa;
...
FROM (SELECT codm, cpf, nome FROM medicos) m
LEFT JOIN (SELECT codm, codp, data FROM consultas) c ON m.codm = c.codm
LEFT JOIN (SELECT codp, nome FROM pacientes) p ON c.codp = p.codp;
